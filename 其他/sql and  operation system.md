...
求交集  intersect
##### 索引类型 普通索引、唯一索引、全文索引、主键索引
索引原理：普通查询是按表的顺序遍历表，找到符合要求的记录。建立索引后，通过bTree算法生成索引文件，通过查找索引文件，找到相应的键从而获得数据

##### 慢查询  
1. 分析慢查询日志    set global slow_query_log='ON'; 开启慢查询日志；set global slow_query_log_file='/usr/local/mysql/data/slow.log';设置慢查询日志存放的位置；set global long_query_time=1;设置超时时间
2. explain关键字  可查看索引是否生效  explain +sql语句 
3. 慢查询优化：建立索引、优化数据库结构、优化limit分页
##### 索引分为 主键索引、唯一性索引、普通索引、组合索引
- 索引的优缺点：
  - 优点：
    1.加快查询速度。
    2.创建唯一索引可确保数据的唯一性。
...
- 创建索引 sql
  - CREATE （类型）INDEX 索引 ON TABLE （列名），列名包含多项时为复合索引
- 查看索引 sql
  - show index from db.table   db.table 指数据库名.数据表名
##### 数据库索引结构b+树
使用自增主键的作用 便于维护索引 
定义二元组（key、value）描述b+树（多叉树），分为根节点、内部节点和叶子节点，内部节点只有key，vaule都存放在叶子节点上，叶子节点在同一层（可增大节点的出度，提高检索性能）。符合二叉搜索树的特点，左子节点的值小于父节点，右子节点的值大于父节点。叶子节点增加一个指向相邻叶子节点的指针，
可提高区间访问的性能。
##### 数据库引擎：MyISAM 和 InnoDB  通过数据库引擎访问数据库文件，引擎定义了数据的存储方式
MyISAM 引擎的索引使用B+树，叶子节点的data存放的是记录的地址  非聚集索引 索引文件和数据文件是分开的  不支持事务，不支持外键
InnoDB  引擎的索引使用B+树，表数据本身按照B+结构组织，key为表主键，vulue为表数据项（主键索引），普通索引的value是主键值，聚集索引  支持事务、支持外键
...
######   数据库索引的原理
#####  数据库视图
1. 概念：通过查询语句建立的虚拟表，不存储数据，只存放视图的定义。单表视图可用用于查询和修改，会修改基本表的数据；多表视图只用于查询
2.作用：
    - 安全性，用户只能查看和修改其能看到的数据
    - 简化操作，把经常使用的数据定义为视图
    - 逻辑上的独立性，屏蔽真实表的结构带来的影响
3. 创建视图 create view 视图名 as select * from 表名 where 条件

##### 关系型数据库vs非关系型数据库
1. 关系型数据库指采用关系模式来组织数据，关系模式是二维表格模型
优点
2. 数据库以键值对存储，结构不固定，每个元组的字段可以不一致
#####  三范式
候选码：能唯一标志一个元组的最小属性集，可以有多个，从中挑选一个作为主键
1. 第一范式： 关系模型中的属性不可再分为更基础的单位时
2. 第二范式： 所有非主属性都完全依赖主键，不存在着主键重复的情况
3. 第三范式： 消除传递依赖 x属性可确定y属性（反之不可行），y属性可确定z属性，那么z传递依赖x属性
### 死锁
...
#### 线程和进程的连续和区别
  ...
  - 线程： cpu调度和分派的基本单位，同一进程中的线程组共享进程所拥有的资源（打开的文件、全局变量、进程代码段，地址空间），但（线程id、栈、寄存器）这些资源是独享的
  ...
  - 区别：
    ...
    3. 进程结束运行时，其拥有的所有线程都将被销毁；线程结束时不会影响到其他线程的结束
    4. 线程需要同步和互斥，因为它们共享进程资源
##### 进程切换开销比线程切换开销  
上下文切换：从当前执行任务切换到另一个执行任务的过程，为了保证下次能从正确的位置运行，需要保存上个任务的状态。通过操作系统内核来完成的。
进程切换开销到是因为要切换虚拟内存，虚拟内存到物理内存的映射是通过页表来实现的，
#### 进程通信的方式
   - 管道
...
- 服务端：
  ...
  4. accept（） 接受客户端的连接请求，返回一个新的套接字用于和客户端通信，阻塞直到有连接，
  5. recv()
- 客户端：
  1. socket（）创建套接字
  2. connect（）请求连接
...
- 终止态   进程结束运行
 ##### 线程状态
 - 创建态  创建了一个线程对象
 - 可运行态 调用线程对象的start()方法，等待被线程调度选中获得cpu
 - 运行态  线程获得cpu时间片，执行程序代码
 - 阻塞态 放弃cpu使用权
 - 死亡态 线程run()、target()方法结束或因发生异常退出执行
